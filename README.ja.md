...
**近道:** リポジトリをクローンしてClaude Codeで開き、`/adopt`を実行してください。これがインタビュー形式で(どのオプション項目を使うか、プラグイン一覧、コピー先パスなど)必要事項を尋ね、下記のステップ2〜6と8〜9を代わりに実行し、進捗を再開可能なジャーナルファイルに記録していきます。ステップ7(プラグインエコシステム自体のインストール)は意図的に`/adopt`の対象外としてあります。これだけは自分で行う必要があります。以下の手動手順は`/adopt`が自動化している内容であり、手作業で進めたい人や、実行前に変更内容を正確に確認したい人のためにも残してあります。

1. **リポジトリをクローン**します。対象マシンの好きな場所で構いません。
2. **オプション項目を今のうちに決めます** — Yes/Noで答えてください。ステップ5で何を削除するかが決まります: ローカルAI(Ollama)による事前圧縮。詳細は下記の「オプション: ___」セクションを参照してください。
3. **`global-config/CLAUDE.md`、`agents/*.md`、`hooks/block-dangerous-git.py`、`skills/*`、`tools/`をコピー**して自分の`~/.claude/`に置きます(マージするか置き換えるかは任意)。これらがあって初めて、ルーティングルール、gitセーフティゲート、スキルカタログ、更新チェッカーが単なる文章ではなく実際に機能します。**`CLAUDE.md`をコピーする前に、「Installed Plugins」セクションを実際に自分が導入しているものだけに書き換えてください** — 元のオーナーのコピーには特定のプラグインが有効になっていると書かれていますが、それをそのまま持ち込むと、Claudeが実際には使えないツールについて嘘をつくことになります。
4. **`global-config/settings.example.json`をマージ**して`~/.claude/settings.json`に反映します(`<YOUR_HOME>`を置き換えた後に。macOS/Linuxではフックコマンドの`py`ランチャーも`python3`に変更してください。同梱の設定はWindows専用として書かれています)。
5. **ステップ2で「いいえ」と答えた場合はOllamaを削除**します。手早く済ませるなら: CLAUDE.mdのコピー内にあるOllamaの段落 + `notes/local-ollama-models.md` + `tools/ollama/`。
6. **残したファイルすべてでプレースホルダーを一括置換**します — 完全なリストは後述の「導入方法」ステップ8を参照してください。
7. **参照されているプラグインエコシステムをインストール**します(superpowers、eccなど) — 詳細は後述の「別途インストールが必要なもの」を参照してください。
8. **必要に応じて`notes/`を自分のセカンドブレイン・ボルトの場所にコピー**し、**`memory-examples/`**を該当プロジェクト用のClaude Code自動メモリフォルダにコピーします。
9. **Claude Codeセッションを開始して確認**します — 新しいCLAUDE.mdが反映されているか、例えば実装計画を依頼して`/plan-pro`が呼ばれるか、あるいはモデルルーティングについて尋ねてコストのはしごが返ってくるかをチェックしてください。
