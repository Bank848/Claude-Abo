...
**지름길:** 저장소를 클론하고 Claude Code에서 열어 `/adopt`를 실행하세요. 어떤 선택 항목을 원하는지, 플러그인 목록, 대상 경로 등을 인터뷰 형식으로 물어보면서 아래 2~6단계와 8~9단계를 대신 처리해 주고, 재개 가능한 저널 파일에 진행 상황을 체크합니다. 7단계(플러그인 생태계 자체를 설치하는 것)는 의도적으로 `/adopt`의 범위 밖에 있습니다. 이건 직접 해야 합니다. 아래 수동 단계는 `/adopt`가 자동으로 처리하는 내용이며, 직접 손으로 하고 싶거나 실행 전에 무엇이 바뀌는지 정확히 확인하고 싶은 사람을 위한 것이기도 합니다.

1. **저장소를 클론**해서 대상 기기의 편한 위치에 둡니다.
2. **선택 항목 하나를 지금 결정**하세요 — 예/아니오로 답하면 되고, 이 답이 5단계에서 무엇을 지울지 결정합니다: 로컬 AI(Ollama) 사전 압축. 자세한 내용은 아래 "선택 사항: ___" 섹션을 참고하세요.
3. **`global-config/CLAUDE.md`, `agents/*.md`, `hooks/block-dangerous-git.py`, `skills/*`, `tools/`를 복사**해서 자신의 `~/.claude/`에 둡니다(병합할지 덮어쓸지는 각자 판단). 이 파일들이 있어야 라우팅 규칙, git 안전장치, 스킬 카탈로그, 업데이트 체커가 단순한 텍스트가 아니라 실제로 동작합니다. **`CLAUDE.md`를 복사하기 전에 "Installed Plugins" 섹션을 반드시 고쳐서 실제로 설치한 것만 적으세요** — 원본에는 원래 소유자가 설치한 특정 플러그인들이 명시되어 있는데, 이를 그대로 가져가면 Claude가 없는 도구를 있다고 착각하게 됩니다.
4. **`global-config/settings.example.json`을 병합**해서 자신의 `~/.claude/settings.json`에 반영합니다(`<YOUR_HOME>`을 바꾼 뒤; macOS/Linux에서는 훅 명령의 `py` 런처도 `python3`로 바꿔야 합니다. 이 항목은 원래 Windows 전용으로 만들어졌습니다).
5. **2단계에서 "아니오"라고 답했다면 Ollama를 삭제**하세요. 빠르게 하려면: CLAUDE.md 사본 안의 Ollama 관련 단락 + `notes/local-ollama-models.md` + `tools/ollama/`를 지우면 됩니다.
6. **남겨둔 파일 전체에서 플레이스홀더를 찾아 바꾸세요** — 전체 목록은 아래 "이 저장소 도입하기"의 8단계를 참고하세요.
7. **참조된 플러그인 생태계를 설치**하세요(superpowers, ecc 등). 아래 "별도로 설치해야 할 것들" 섹션을 참고하세요.
8. **필요하면 `notes/`를 자신의 세컨드 브레인 볼트 위치로 복사**하고, **`memory-examples/`는 해당 프로젝트의 Claude Code 자동 메모리 폴더로 복사**하세요.
9. **Claude Code 세션을 새로 시작해서 새 CLAUDE.md가 반영됐는지 확인**하세요 — 예를 들어 구현 계획을 요청해서 `/plan-pro`가 호출되는지 보거나, 모델 라우팅에 대해 물어봐서 비용 사다리가 제대로 나오는지 확인하면 됩니다.
