...
description: Reference for 26 web2 bug classes with root causes, detection patterns, and remediation notes, for authorized security testing and defensive review. Covers IDOR, auth/MFA bypass, XSS, SSRF, SQLi, business logic, race conditions, OAuth/OIDC, file upload, GraphQL, LLM/AI agentic risks, API misconfig (mass assignment, JWT, prototype pollution, CORS), account-takeover taxonomy, SSTI, subdomain takeover, cloud/infra misconfig, HTTP request smuggling, cache poisoning, SAML attacks, error/debug disclosure, CSS injection, file inclusion, insecure deserialization, and dependency confusion / supply chain. Use when studying or hunting a specific vuln class.
---
