...
3. **Suno music-production notes** — keep or delete? (See README "Optional: Suno AI music production notes".) If keeping, ask for their real songs directory path to fill in `<YOUR_SONGS_DIR>` — or "skip" to leave it as a placeholder for later.
4. **Second-brain vault path** — for `<YOUR_VAULT_PATH>`. "Skip" is fine.
5. **Username / home path** — for `<YOUR_USERNAME>` / `<YOUR_HOME>`. Prefer auto-detecting from the environment and just confirming, same as Q1.
6. **Which of these plugins do you already have installed (or plan to install right now)?** List them explicitly: `superpowers`, `ecc` (everything-claude-code), `pordee`, `lazyweb`, `andrej-karpathy-skills`. This answer controls what the copied CLAUDE.md is allowed to claim — see Step 4's plugin-honesty fix, which is the most important correctness step in this whole command.
...
6. Merge `global-config/settings.example.json` into `<target>/settings.json`: read the existing file if present, merge in the `hooks.PreToolUse` entry and whichever `enabledPlugins` entries match Step 2 Q6's answer — do not blind-overwrite an existing settings.json. After writing, **parse the result as JSON before considering the step done**; if it fails to parse, revert to the pre-edit content, report the failure, and stop (do not continue to later steps with a broken settings.json).
...
9. If a vault path was given: copy `notes/` there (skip entirely if Suno was declined and that's the only content the user cares about — ask if unsure). If a songs directory was given, replace `<YOUR_SONGS_DIR>` in the copied `notes/suno-music-production/**` files; otherwise leave the placeholder and flag it in the final report.
10. Apply the remaining placeholder find-and-replace (`<YOUR_USERNAME>`, `<YOUR_HOME>`, `<YOUR_VAULT_PATH>`) across whatever was actually copied in steps 1-9 — do not touch files that were skipped/deleted per the user's yes/no answers. Before running a broad find-and-replace, `grep` the target files first so you know the real count going in — don't trust any number written in this file or in README.md, they can go stale; verify live, every time.
